...
WorkstationRoutes.get(PATH, async (request, response) => {
  try {
    const fetchAllUC = new FetchAll(
      new MongoDBWorkstationService(new MongoDBConnection())
    )
    const allWorkstation = await fetchAllUC.execute()
...
WorkstationRoutes.post(
  ...
  async (
    ...
    try {
      ...
      const createdWorkstation = await createUC.execute({value})
...
WorkstationRoutes.get(
  ...
  async (
    ...
    try {
      ...
      const foundWorkstation = await findByIdUC.execute(id)
...
WorkstationRoutes.patch(
  ...
  async (
    ...
    try {
      ...
      const updatedWorkstation = await updateUC.execute(id, newWorkstation)
